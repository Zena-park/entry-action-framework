...
**Impossibility Result - Correcting 15 Years of Fundamental Errors:** This work mathematically proves that vouching network research over the past 15 years, represented by SybilGuard, SybilLimit, and SybilRank, contains **critical flaws**. These studies claimed that PageRank-based algorithms provide Sybil resistance under the assumption that "attack edges between honest and malicious regions are limited," but this rests on **two fundamental impossibilities**: (1) **Graph Isomorphism (Theorem 3.1)**: When attackers socially engineer honest users to obtain vouches, the resulting graph is structurally indistinguishable from honest networks—no graph-based algorithm can distinguish isomorphic structures by mathematical definition; (2) **Semantic Labeling (Theorem 5.4)**: Even if community detection algorithms (Louvain, Infomap) successfully partition the graph in polynomial time, they cannot determine which clusters are malicious without external information—this is a semantic ambiguity problem, not a computational complexity issue. We formally prove that Sybil resistance is **fundamentally impossible** in pure address-based systems without external identity verification. This is not merely an empirical observation that "attacks are difficult," but an **Impossibility Result** showing that an entire approach does not work in principle, correcting the direction of vouching network research so it no longer pursues illusions.
...
**This work suggests that the quest for a purely algorithmic Sybil defense has been a "Godot-like" pursuit for the past 15 years, and we provide the necessary mathematical closure to pivot the field toward context-aware social infrastructure.**
...
1. **Graph Isomorphism - Structural Indistinguishability** (Theorem 3.1): If attackers with sufficient resources socially engineer honest users to obtain vouches, the resulting graph has a **graph isomorphism** relationship with honest networks. By mathematical definition, **no graph-based algorithm can distinguish isomorphic structures**—this is not a computational limitation but a fundamental impossibility.

2. **Semantic Labeling Problem** (Theorem 5.4): Even if community detection algorithms like Louvain or Infomap successfully partition the graph into clusters in polynomial time, they cannot determine **which clusters are malicious and which are legitimate**. Without external information, this labeling is **semantically ambiguous**—both labelings (honest vs. Sybil) are equally valid from a graph-structural perspective.
...
1. **Theorem 3.1 (Fundamental Limitation - Graph Isomorphism)**: We formally prove that Sybil resistance is **mathematically impossible** in pure address-based systems without external identity verification (§3.3)
   ...
   - By mathematical definition, **no graph-based algorithm can distinguish isomorphic structures**—this is not a computational limitation but a structural impossibility
   - The theoretical foundation of SybilGuard, SybilLimit, and SybilRank collapses

2. **Theorem 5.4 (Semantic Labeling Impossibility)**: We prove that the theoretical bounds of previous studies (e.g., "total score of malicious set ≤ m") are **infeasible circular logic** (§5.3)
   - **Core of circular logic**: To apply the bound, one must first identify the malicious subset $S$
   - **Polynomial-time clustering is insufficient**: Community detection algorithms like Louvain or Infomap can partition the graph in polynomial time, but they cannot determine **which clusters are malicious**
   - **Semantic ambiguity**: Without external information, both labelings (honest vs. Sybil) are equally valid from a graph-structural perspective—this is a **semantic labeling problem**, not merely a computational complexity issue
...
- **Theorem B.9**: Quantifies Sybil resistance of entry conditions via ROI ($R(\phi) = \min\{n_m : \text{ROI} > 0\}$)
...
**Theorem 5.4 (Semantic Labeling Impossibility)**: *Theorem 5.3 only holds when the Sybil subset $S$ can be identified. Without external information, no graph-based algorithm can determine which communities are malicious vs. legitimate—this is a semantic labeling problem, not merely a computational complexity issue.*
...
- Network 1: 100 real users who know each other (densely connected community)
- Network 2: 100 addresses controlled by an attacker with the same graph structure (isomorphic Sybil cluster)

**Key insight**: Community detection algorithms (Louvain, Infomap, etc.) can successfully find both clusters in polynomial time. However, they cannot determine **which cluster is malicious**.

By graph isomorphism, $\mathcal{A}$ must behave identically on both inputs. Therefore, $\mathcal{A}$ cannot **semantically label** which communities are legitimate vs. Sybil. Both interpretations (Graph 1 = honest, Graph 2 = Sybil) and (Graph 1 = Sybil, Graph 2 = honest) are structurally equivalent. ∎

**Conclusion**: The problem is not finding communities (solvable in polynomial time), but **determining their semantic meaning** (malicious vs. honest) without external information. Complex scoring provides theoretical bounds that cannot actually be enforced.
...
- ✅ **Dynamic graph analysis added (Appendix B.4.2)**: This limitation is resolved by formalizing temporal score evolution in dynamic vouching graphs (Theorem B.7: Bootstrapping Problem, Theorem B.8: Detection vs Prevention), proving that dynamic models shift the impossibility from graph isomorphism to initial state dependency
...
- **Recognition that the 15-year quest for purely algorithmic Sybil defense has been a "Godot-like" pursuit**, and this work provides the necessary mathematical closure to redirect the field toward context-aware social infrastructure
...
#### B.3.2 Equilibrium for PageRank-based Entry Condition

**Theorem B.5 (Complexity of PageRank Entry Conditions)**:
When the entry condition is $\phi_p(G, v) = (s_{PR}(v) \geq \theta)$ (PageRank score):

1. **Pure strategy Nash equilibrium does not always exist**
2. **Mixed strategy equilibrium exists but is computationally difficult**
3. **In practice, heuristic-based approximate equilibrium**

**Proof sketch**:

**Step 1**: Interdependence

PageRank depends on the entire graph structure:
$$s_{PR}(v) = \frac{1-d}{N} + d \sum_{u \in \text{In}(v)} \frac{s_{PR}(u)}{|\text{Out}(u)|}$$

Player $i$'s vouching action $a_i$:
- Directly affects PageRank of elements in $a_i$
- Indirectly affects PageRank distribution of entire network

**Step 2**: Circular dependency

For $v$'s score to be high → must receive vouches from high-scoring $u$ → for $u$'s score to be high ... (circular)

This makes the strategy space complex and does not guarantee existence of pure strategy equilibrium.

**Step 3**: Computational complexity

For each player to compute best response:
- Must calculate PageRank for all possible vouch combinations
- $O(2^{k_{max}} \cdot N \cdot E)$ - practically infeasible

Therefore, in practice, approximate algorithms or heuristics are used.

∎

### B.4 Repeated Games, Reputation Effects, and Dynamic Models

#### B.4.1 Infinite Repeated Game Model

**Definition B.5 (Repeated Vouching Game)**:
When vouching game $\Gamma$ is repeated infinitely, at each round $t = 1, 2, ...$:
- Players simultaneously choose vouching actions $a_i^t$
- Receive immediate utility $u_i(\mathbf{a}^t)$
- Future utilities are discounted by discount factor $\delta \in (0, 1)$

Total utility:
$$U_i = \sum_{t=1}^\infty \delta^{t-1} \cdot u_i(\mathbf{a}^t)$$

**Theorem B.6 (Folk Theorem for Vouching)**:
When discount factor $\delta$ is sufficiently high ($\delta > \delta^*$), there exists a Subgame Perfect Equilibrium satisfying:

1. **Honest vouching is maintained on equilibrium path**
2. **Deviation is punished by permanent vouch withdrawal**

**Proof sketch**:

**Step 1**: Define trigger strategy

Each player $i$'s strategy:
- **Cooperation phase**: Vouch only for actual trust relationships ($a_i = T_i$)
- **Punishment phase**: Withdraw vouches from $j$ who made false vouches or violated reciprocity in the past

**Step 2**: Analyze deviation incentive

When player $i$ deviates from cooperation by choosing $a_i' \neq T_i$:
- Immediate gain: $\Delta u_i$ (cost savings or additional entry)
- Future loss: Other players withdraw vouches from $i$ → entry failure

Present value of future loss:
$$PV(\text{Loss}) = \sum_{t=1}^\infty \delta^t \cdot b_i = \frac{\delta \cdot b_i}{1 - \delta}$$

Deviation prevention condition:
$$\Delta u_i < \frac{\delta \cdot b_i}{1 - \delta}$$

When $\delta$ is large enough ($\delta > \delta^* = \frac{\Delta u_i}{\Delta u_i + b_i}$), deviation is irrational.

**Step 3**: Reputation mechanism

In repeated games, reputation forms such that:
- Honest vouchers are trusted and continue to receive vouches
- False vouchers lose reputation and fail to receive future vouches

∎

#### B.4.2 Dynamic Graph Model and Temporal Score Evolution

**Motivation**: Theorem 3.1 proved the limitations of scoring functions that analyze only graph structure. This assumes a **static graph snapshot**. A natural question arises: **Can we overcome this limitation by considering graph evolution over time?** This subsection formalizes the dynamic graph model and demonstrates that the fundamental impossibility persists due to the **bootstrapping problem**.

**Definition B.6 (Dynamic Vouching Graph)**:
For time $t \in \mathbb{N}$, a dynamic vouching graph is a sequence $\{G_t\}_{t=0}^\infty$ where:
- $G_t = (V_t, E_t)$ is the graph at time $t$
- $V_t \subseteq V_{t+1}$ (nodes can only be added, monotonic growth)
- $E_t \subseteq V_t \times V_t$ (edges at time $t$ are relationships between nodes at time $t$)
- No inclusion relationship between $E_t$ and $E_{t+1}$ (edges can be added or removed)

**Definition B.7 (Temporal Scoring Function)**:
A dynamic scoring function $s: V \times \mathbb{N} \rightarrow \mathbb{R}^+$ has the form:

$$s(v, t) = f(s(v, t-1), G_t, H_{t-1})$$

where:
- $s(v, t-1)$: Previous score (historical reputation)
- $G_t$: Current graph structure
- $H_{t-1} = \{G_0, G_1, ..., G_{t-1}\}$: Historical graph sequence

**Examples**:
- **Decay model**: $s(v, t) = \alpha \cdot s(v, t-1) + \beta \cdot s_{\text{static}}(v, G_t)$ where $\alpha + \beta = 1$
- **Cumulative model**: $s(v, t) = \sum_{\tau=0}^t \gamma^\tau \cdot w(v, G_\tau)$
- **Threshold model**: $s(v, t) = s(v, t-1) + \Delta$ if new vouch added, else $s(v, t-1) \cdot \lambda$ (decay)

**Theorem B.7 (Bootstrapping Problem - Initial State Dependency)**:
*Every dynamic scoring function $s(v, t)$ depends on initial state $s(v, 0)$ or an initial trusted set $T_0 \subset V_0$. Setting $s(v, 0)$ or $T_0$ without external verification reduces to the impossibility result of Theorem 3.1.*

**Proof**:

**Case 1: Uniform Initialization** ($s(v, 0) = c$ for all $v \in V_0$)

Assume initial scores are identical for all nodes. An attacker can create $k$ sybil addresses at $t=0$:

$$s(A_i, 0) = c = s(h, 0) \quad \forall \text{honest node } h, \text{ sybil } A_i$$

**Assumption**: Suppose the scoring function $f$ depends only on structural graph properties. That is, $f(s, G, H)$ uses only structural features (degree, neighbor score distribution, clustering coefficient, etc.) rather than node identity.

Temporal evolution is defined recursively:
$$s(v, t) = f(s(v, t-1), G_t, H_{t-1})$$

**Proof by induction**:

- **Base case** ($t=0$): $s(A_i, 0) = c = s(h_i, 0)$ (by assumption)

- **Inductive step**: Assume $s(v, \tau) = s(u, \tau)$ holds for nodes $v$ and $u$ that correspond under graph isomorphism.

  If the attacker constructs an **isomorphic subgraph** through sybil-to-sybil vouches (construction from Theorem 3.1):

  $$G_{\text{sybil}}(\tau+1) \cong G_{\text{honest}}(\tau+1)$$

  Since $f$ depends only on structural properties, for graph isomorphism $\phi: V_{\text{honest}} \to V_{\text{sybil}}$:

  $$f(s(h_i, \tau), G_{\text{honest}}(\tau+1), H_{\tau}) = f(s(\phi(h_i), \tau), G_{\text{sybil}}(\tau+1), H_{\tau})$$

  Therefore:
  $$s(h_i, \tau+1) = s(A_i, \tau+1)$$

- **Conclusion**: $s(A_i, t) = s(h_i, t) \quad \forall t$ (by mathematical induction)

Thus, uniform initialization provides no sybil resistance. ✗

**Case 2: Trusted Seed Set** ($s(v, 0) = \begin{cases} c_{\text{high}} & v \in T_0 \\ c_{\text{low}} & v \notin T_0 \end{cases}$)

Assume an initial trusted set $T_0$ is given. The critical question:

> **"How do we choose $T_0$?"**

**Subcase 2a**: $T_0$ selected by **on-chain verifiable criteria** (e.g., initial deployer, token holders)
- This is an **external verification mechanism** (violates Theorem 3.1 premise)
- Examples: WorldID, token gating, POAP

**Subcase 2b**: $T_0$ selected by **graph-based heuristics**
- At $t=0$, graph is empty or initial → Theorem 3.1 applies
- Attacker can mimic honest patterns from $t=0$

**Subcase 2c**: $T_0$ **manually curated** (e.g., designated by administrator)
- This is **centralized identity verification** (violates decentralization)

**Conclusion**: In all cases, setting $T_0$ reduces to external verification, centralization, or the impossibility of Theorem 3.1. ∎

**Theorem B.8 (Practical Value of Dynamic Models - Detection vs Prevention)**:
*Dynamic graph models enable sybil **detection and removal** rather than sybil **prevention**.*

**Proof sketch**:

**Scenario**: At time $t=0$, trusted set $T_0$ is established (via external mechanism).

**Observable patterns at $t \geq 1$**:

1. **Normal growth**: Honest nodes receive vouches from diverse existing nodes over time
   $$\frac{d|N^-(h, t)|}{dt} \approx \text{const}, \quad N^-(h, t) \cap T_t \neq \emptyset$$

2. **Sybil pattern**: New sybil clusters appear suddenly and vouch primarily among themselves
   $$\frac{|N^-(A_i, t) \cap S|}{|N^-(A_i, t)|} \approx 1, \quad \text{where } S = \{A_1, ..., A_k\}$$

   That is, most of a sybil's vouches come from other sybils.

**Detection metric**:
$$\text{SybilScore}(v, t) = \begin{cases}
\frac{|\{u \in N^-(v, t) : s(u, t) < \theta_{\text{low}}\}|}{|N^-(v, t)|} & \text{if } |N^-(v, t)| > 0 \\
0 & \text{otherwise}
\end{cases}$$

High SybilScore → suspicious node → removal candidate

**However, limitations**:

- **Slow Sybil Attack**: If attacker adds sybils gradually over long time and obtains vouches from honest users, detection is impossible
- **Social Engineering**: If attacker deceives honest users into vouching, indistinguishable (Theorem 3.1)

∎

**Corollary B.8.1 (Position of Dynamic Models)**:
*Dynamic graph models do not break the impossibility of Theorem 3.1. Instead, they are useful in practical systems for:*

1. **Rapid attack detection**: Detecting mass account creation in short time
2. **Reputation accumulation**: Granting higher trust to older accounts
3. **Behavioral pattern analysis**: Identifying abnormal vouching patterns
4. **Gradual trust building**: New participants earn reputation over time

**However, all of this presumes an initial trust anchor $T_0$ or external verification.**

#### B.4.3 Static vs Dynamic: Comparative Analysis

| Aspect | Static Graph (Theorem 3.1) | Dynamic Graph (This Section) |
|--------|---------------------------|------------------------------|
| **Analysis target** | Single snapshot $G_t$ | Sequence $\{G_0, ..., G_t\}$ |
| **Scoring** | $s(v) = f(G_t)$ | $s(v, t) = f(s(v,t-1), G_t, H_{t-1})$ |
| **Impossibility** | Graph isomorphism → indistinguishable | Bootstrapping → initialization impossible |
| **External verification needed** | ✓ (sybil defense impossible) | ✓ (initial trust anchor needed) |
| **Practical value** | Simple reputation system | Detection + temporal trust |
| **Attack resistance** | Vulnerable to isomorphic graphs | Vulnerable to slow attacks |

**Key insight**:
- Static impossibility (Theorem 3.1): **"We cannot know who is sybil"**
- Dynamic impossibility (Theorem B.7): **"We cannot know whom to trust initially"**

Both impossibilities are different expressions of the **same fundamental problem**: **address-based systems cannot achieve sybil resistance without external identity verification**.

#### B.4.4 Practical Implications

**Recommendations**:

1. **Be transparent about initial trust setup**:
   - Do not claim "fully decentralized sybil defense"
   - Explicitly state initial trust anchor (e.g., "token holders", "POAP holders")

2. **Use dynamic models as auxiliary tools**:
   - Primary: Entry condition ($\phi$) - context definition
   - Secondary: Dynamic score ($s(v, t)$) - reputation accumulation

3. **Employ as detection system**:
   - Real-time monitoring to detect rapid attacks
   - Periodic audits to remove suspicious patterns

**Application in SYB System**:

Current SYB implementation primarily uses static scoring, but dynamic extension is possible:

```solidity
// Future extension example (currently unimplemented)
struct DynamicScore {
    uint256 currentScore;      // s(v, t)
    uint256 previousScore;     // s(v, t-1)
    uint256 lastUpdateTime;    // t
    uint256 accountAge;        // t - t_creation
}
```

This enables:
- Decay rate for old accounts: $s(v, t) = s(v, t-1) \cdot e^{-\lambda \Delta t} + \Delta s$
- Detecting sudden score changes: $|s(v, t) - s(v, t-1)| > \theta_{\text{alarm}}$

**However, this is meaningful only when entry condition $\phi$ correctly defines the initial cohort (Entry Action Framework).**

### B.5 ROI-Based Sybil Resistance Quantification

#### B.5.1 Attack ROI Definition

**Definition B.8 (Attack ROI)**:
...
#### B.5.2 Concrete ROI Calculations
...
#### B.5.3 Formal Theorem

**Theorem B.9 (ROI-Based Sybil Resistance)**:
...
### B.6 Mechanism Design Principles
