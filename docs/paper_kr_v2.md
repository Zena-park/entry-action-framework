...
**불가능성 결과(Impossibility Result) - 15년 연구의 근본적 오류 교정:** 본 연구는 SybilGuard, SybilLimit, SybilRank로 대표되는 지난 15년간의 보증 네트워크 연구가 **결정적인 결함**을 내포함을 수학적으로 증명한다. 이들 연구는 "정직한 영역과 악의적 영역 사이의 공격 엣지가 제한적"이라는 가정 하에서 PageRank 기반 알고리즘이 시빌 저항을 제공한다고 주장했으나, 이는 **두 가지 근본적 불가능성**에 직면한다: (1) **그래프 동형성 (정리 3.1)**: 공격자가 정직한 사용자들을 소셜 엔지니어링으로 속여 보증을 획득하면, 생성된 그래프는 정직한 네트워크와 구조적으로 구별 불가능—수학적 정의에 따라 어떤 그래프 기반 알고리즘도 동형 구조를 구별할 수 없음; (2) **의미론적 라벨링 (정리 5.4)**: Louvain, Infomap 같은 커뮤니티 탐지 알고리즘이 다항 시간에 그래프를 성공적으로 분할하더라도, 외부 정보 없이는 어느 클러스터가 악의적인지 판단 불가능—이는 의미론적 모호성 문제이지, 계산 복잡도 이슈가 아님. 우리는 외부 신원 검증 없이는 순수 주소 기반 시스템에서 시빌 저항이 **원리적으로 불가능**함을 형식적으로 증명한다. 이는 단순히 "공격이 어렵다"는 경험적 관찰이 아니라, 특정 접근법 전체가 작동하지 않음을 보이는 **Impossibility Result**로서, 보증 네트워크 연구가 더 이상 허상을 좇지 않도록 방향을 교정한다.
...
**본 연구는 지난 15년간의 순수 알고리즘적 시빌 방어 탐구가 '고도를 기다리며'와 같은 추구였음을 시사하며, 분야가 맥락 인식 사회 인프라로 전환하는 데 필요한 수학적 마무리를 제공한다.**
...
1. **그래프 동형성 - 구조적 구별 불가능성** (정리 3.1): 충분한 자원을 가진 공격자가 정직한 사용자들을 소셜 엔지니어링으로 속여 보증을 획득하면, 생성된 그래프는 정직한 네트워크와 **그래프 동형(graph isomorphism)** 관계를 갖는다. 수학적 정의에 따라, **어떤 그래프 기반 알고리즘도 동형 구조를 구별할 수 없다**—이는 계산 복잡도의 한계가 아니라 원리적 불가능성이다.

2. **의미론적 라벨링 문제** (정리 5.4): Louvain이나 Infomap 같은 커뮤니티 탐지 알고리즘이 다항 시간에 성공적으로 그래프를 클러스터로 분할하더라도, **어느 클러스터가 악의적이고 어느 클러스터가 정당한지 판단할 수 없다**. 외부 정보 없이는 이러한 라벨링이 **의미론적으로 모호**하다—그래프 구조적 관점에서 두 라벨링(정직 vs 시빌) 모두 동등하게 타당하다.
...
**본 연구의 핵심 기여는 이러한 한계가 단순히 "구현의 어려움"이 아니라 "원리적 불가능성"임을 수학적으로 증명하는 것이다.** 우리는 외부 신원 검증 메커니즘 없이는 순수한 주소 기반 보증 시스템이 다중 계정 공격에 대한 완전한 저항성을 보장할 수 없음을 형식적으로 증명한다 (§3.3, 정리 5.4). 이는 암호학에서 특정 프로토콜의 안전성을 증명하는 것이 아니라, 특정 접근법 전체가 원리적으로 작동하지 않음을 보이는 **Impossibility Result**이다. SybilGuard 이후 15년간 학계가 추구해온 "소셜 그래프 분석만으로 시빌 방어"라는 목표는 **달성 불가능한 허상**이었으며, 본 연구는 이를 명확히 밝힘으로써 향후 연구가 더 이상 잘못된 방향으로 향하지 않도록 교정한다.
...
1. **정리 3.1 (근본적 한계 - Graph Isomorphism)**: 외부 신원 검증 없이는 순수 주소 기반 시스템에서 시빌 저항이 **수학적으로 불가능**함을 형식적으로 증명 (§3.3)
   ...
   - 수학적 정의에 따라, **어떤 그래프 기반 알고리즘도 동형 구조를 구별할 수 없음**—이는 계산 복잡도의 한계가 아니라 구조적 불가능성
   - SybilGuard, SybilLimit, SybilRank의 이론적 기반이 붕괴됨

2. **정리 5.4 (의미론적 라벨링 불가능성)**: 기존 연구들의 이론적 경계(예: "악의적 집합의 총 점수 ≤ m")가 **실현 불가능한 순환 논리**임을 증명 (§5.3)
   - **순환 논리의 핵심**: 경계를 적용하려면 먼저 악의적 부분집합 $S$를 식별해야 함
   - **다항 시간 클러스터링의 한계**: Louvain이나 Infomap 같은 커뮤니티 탐지 알고리즘은 다항 시간에 그래프를 분할할 수 있지만, **어느 클러스터가 악의적인지는 판단 불가능**
   - **의미론적 모호성**: 외부 정보 없이는 두 라벨링(정직 vs 시빌) 모두 그래프 구조적 관점에서 동등하게 타당함—이는 **의미론적 라벨링 문제**이지, 단순한 계산 복잡도 이슈가 아님
...
**정리 5.4 (의미론적 라벨링 불가능성)**: *정리 5.3은 시빌 부분집합 $S$를 식별할 수 있을 때만 성립한다. 외부 정보 없이는 어떤 그래프 기반 알고리즘도 어느 커뮤니티가 악의적이고 어느 커뮤니티가 정당한지 판단할 수 없다—이는 의미론적 라벨링 문제이지, 단순한 계산 복잡도 이슈가 아니다.*

**증명**: 그래프 구조만 사용하여 시빌 부분집합을 식별하는 알고리즘 $\mathcal{A}$가 존재한다고 가정하자. 두 네트워크를 고려하라:
- 네트워크 1: 서로를 아는 100명의 진짜 사용자 (밀집 연결된 커뮤니티)
- 네트워크 2: 동일한 그래프 구조를 가진 공격자가 제어하는 100개의 주소 (동형 시빌 클러스터)

**핵심 통찰**: Louvain, Infomap 등의 커뮤니티 탐지 알고리즘은 다항 시간에 두 클러스터를 성공적으로 찾을 수 있다. 그러나 **어느 클러스터가 악의적인지는 판단할 수 없다**.

그래프 동형성에 의해, $\mathcal{A}$는 두 입력에 동일하게 동작해야 한다. 따라서 $\mathcal{A}$는 어느 커뮤니티가 정당하고 어느 커뮤니티가 시빌인지 **의미론적으로 라벨링**할 수 없다. 두 해석 (그래프 1 = 정직, 그래프 2 = 시빌)과 (그래프 1 = 시빌, 그래프 2 = 정직) 모두 구조적으로 동등하다. ∎

**결론**: 문제는 커뮤니티를 찾는 것(다항 시간에 해결 가능)이 아니라, **의미론적 의미를 결정**하는 것(외부 정보 없이는 불가능)이다. 복잡한 스코어링은 실제로 강제할 수 없는 이론적 경계를 제공한다.
...
- ✅ **동적 그래프 분석 추가 (부록 B.4.2)**: 이 한계는 동적 보증 그래프에서의 시간적 점수 진화를 형식화함으로써 해결되었다 (정리 B.7: 부트스트랩 문제, 정리 B.8: 탐지 vs 예방), 동적 모델이 불가능성을 그래프 동형에서 초기 상태 의존성으로 이동시킴을 증명
...
- **15년간의 순수 알고리즘적 시빌 방어 탐구가 '고도를 기다리며'와 같은 추구였음을 인식**하며, 본 연구는 분야가 맥락 인식 사회 인프라로 방향을 전환하는 데 필요한 수학적 마무리를 제공함
...
#### B.4.2 동적 그래프 모델과 시간적 점수 진화

**동기**: 정리 3.1은 그래프 구조만을 분석하는 스코어링 함수의 한계를 증명했다. 이는 **static graph snapshot**을 가정한 것이다. 자연스러운 질문은: **시간에 따른 그래프 진화를 고려하면 이 한계를 극복할 수 있는가?** 본 섹션은 dynamic graph model을 형식화하고, **bootstrapping problem**으로 인해 근본적 불가능성이 지속됨을 보인다.

**정의 B.6 (동적 보증 그래프)**:
시간 $t \in \mathbb{N}$에 대해, 동적 보증 그래프는 시퀀스 $\{G_t\}_{t=0}^\infty$이다. 여기서:
- $G_t = (V_t, E_t)$는 시점 $t$의 그래프
- $V_t \subseteq V_{t+1}$ (노드는 추가만 가능, 단조 증가)
- $E_t \subseteq V_t \times V_t$ (각 시점의 엣지는 해당 시점 노드 사이의 관계)
- $E_t$와 $E_{t+1}$ 간 포함 관계 없음 (엣지 추가/삭제 모두 가능)

**정의 B.7 (시간적 스코어링 함수)**:
동적 스코어링 함수 $s: V \times \mathbb{N} \rightarrow \mathbb{R}^+$는 다음 형태를 갖는다:

$$s(v, t) = f(s(v, t-1), G_t, H_{t-1})$$

여기서:
- $s(v, t-1)$: 이전 시점의 점수 (역사적 평판)
- $G_t$: 현재 그래프 구조
- $H_{t-1} = \{G_0, G_1, ..., G_{t-1}\}$: 과거 그래프 히스토리

**예시**:
- **감쇠 모델**: $s(v, t) = \alpha \cdot s(v, t-1) + \beta \cdot s_{\text{static}}(v, G_t)$ where $\alpha + \beta = 1$
- **누적 모델**: $s(v, t) = \sum_{\tau=0}^t \gamma^\tau \cdot w(v, G_\tau)$
- **임계값 모델**: $s(v, t) = s(v, t-1) + \Delta$ if 새로운 보증 추가, else $s(v, t-1) \cdot \lambda$ (감쇠)

**정리 B.7 (Bootstrapping Problem - 초기 상태 의존성)**:
*모든 동적 스코어링 함수 $s(v, t)$는 초기 상태 $s(v, 0)$ 또는 초기 신뢰 집합 $T_0 \subset V_0$에 의존한다. 외부 검증 없이 $s(v, 0)$ 또는 $T_0$를 설정하는 것은 정리 3.1의 불가능성 결과로 귀결된다.*

**증명**:

**Case 1: Uniform Initialization** ($s(v, 0) = c$ for all $v \in V_0$)

초기 점수가 모든 노드에 대해 동일하다고 가정하자. 공격자는 $t=0$에 $k$개의 시빌 주소 생성 가능:

$$s(A_i, 0) = c = s(h, 0) \quad \forall \text{정직한 노드 } h, \text{ 시빌 } A_i$$

**가정**: 스코어링 함수 $f$가 그래프 구조적 속성에만 의존한다고 가정하자. 즉, $f(s, G, H)$는 노드의 신원(identity)이 아니라 구조적 특징(degree, 이웃 점수 분포, 클러스터링 계수 등)만 사용한다.
...
- **Base case** ($t=0$): $s(A_i, 0) = c = s(h_i, 0)$ (가정에 의해)

- **Inductive step**: $s(v, \tau) = s(u, \tau)$가 성립한다고 가정하자 (여기서 $v$와 $u$는 그래프 동형 사상에서 대응되는 노드).

  공격자가 시빌 간 보증을 통해 정직한 그래프와 **동형인 부분그래프**를 구축하면 (정리 3.1의 구성):

  $$G_{\text{sybil}}(\tau+1) \cong G_{\text{honest}}(\tau+1)$$

  $f$가 구조적 속성에만 의존하므로, 그래프 동형 사상 $\phi: V_{\text{honest}} \to V_{\text{sybil}}$에 대해:
...
- **결론**: $s(A_i, t) = s(h_i, t) \quad \forall t$ (수학적 귀납법에 의해)

즉, uniform initialization은 시빌 저항을 제공하지 못한다. ✗

**Case 2: Trusted Seed Set** ($s(v, 0) = \begin{cases} c_{\text{high}} & v \in T_0 \\ c_{\text{low}} & v \notin T_0 \end{cases}$)

초기 신뢰 집합 $T_0$가 주어졌다고 가정하자. 이제 핵심 질문:

> **"어떻게 $T_0$를 선택하는가?"**

**Subcase 2a**: $T_0$가 **온체인 검증 가능한 기준**으로 선택됨 (예: 초기 deployer, 토큰 보유자)
- 이것은 **외부 검증 메커니즘**이다 (정리 3.1의 전제 조건 위배)
- 예: WorldID, 토큰 게이팅, POAP 등

**Subcase 2b**: $T_0$가 **그래프 구조 기반 휴리스틱**으로 선택됨
- $t=0$에서는 그래프가 비어있거나 초기 상태 → 정리 3.1 적용
- 공격자는 $t=0$부터 정직한 패턴을 모방 가능

**Subcase 2c**: $T_0$가 **수동으로 큐레이션**됨 (예: 관리자가 지정)
- 이것은 **중앙화된 신원 검증**이다 (탈중앙화 위배)

**결론**: 모든 경우에서 $T_0$ 설정은 외부 검증, 중앙화, 또는 정리 3.1의 불가능성으로 귀결된다. ∎

**정리 B.8 (동적 모델의 실제 가치 - Detection vs Prevention)**:
*동적 그래프 모델은 시빌 **prevention**(사전 방지)이 아니라 시빌 **detection and removal**(탐지 및 제거)를 가능하게 할 수 있다.*

**증명 스케치**:

**시나리오**: 시점 $t=0$에 신뢰 집합 $T_0$가 (외부 메커니즘으로) 설정됨.

**시점 $t \geq 1$에서 관찰 가능한 패턴**:

1. **정상적 성장**: 정직한 노드는 시간에 따라 다양한 기존 노드로부터 보증을 받음
   $$\frac{d|N^-(h, t)|}{dt} \approx \text{const}, \quad N^-(h, t) \cap T_t \neq \emptyset$$

2. **시빌 패턴**: 새로운 시빌 클러스터는 급격히 나타나고 주로 서로만 보증
   $$\frac{|N^-(A_i, t) \cap S|}{|N^-(A_i, t)|} \approx 1, \quad \text{where } S = \{A_1, ..., A_k\}$$

   즉, 시빌의 보증 중 대부분이 다른 시빌로부터 온다.
...
높은 SybilScore → 의심스러운 노드 → 제거 후보

**그러나 한계**:

- **Slow Sybil Attack**: 공격자가 천천히, 오랜 시간에 걸쳐 시빌을 추가하고 정직한 사용자로부터 보증을 획득하면 탐지 불가능
- **Social Engineering**: 공격자가 정직한 사용자를 속여 보증을 받으면 구별 불가능 (정리 3.1)

∎

**따름정리 B.8.1 (Dynamic Model의 위치)**:
*동적 그래프 모델은 정리 3.1의 불가능성을 깨지 못한다. 대신, 실용적 시스템에서 다음 용도로 유용하다:*

1. **급격한 공격 탐지**: 짧은 시간에 대량 계정 생성 감지
2. **평판 누적**: 오래된 계정에 더 높은 신뢰 부여
3. **행동 패턴 분석**: 비정상적 보증 패턴 식별
4. **점진적 신뢰 구축**: 신규 참여자가 시간에 걸쳐 평판 획득

**그러나 이 모든 것은 초기 신뢰 앵커 $T_0$ 또는 외부 검증을 전제로 한다.**

#### B.4.3 Static vs Dynamic: 비교 분석

| 측면 | Static Graph (정리 3.1) | Dynamic Graph (본 섹션) |
|------|------------------------|------------------------|
| **분석 대상** | 단일 시점 $t$의 그래프 $G_t$ | 시퀀스 $\{G_0, ..., G_t\}$ |
| **스코어링** | $s(v) = f(G_t)$ | $s(v, t) = f(s(v,t-1), G_t, H_{t-1})$ |
| **불가능성** | 그래프 동형 → 구별 불가능 | Bootstrapping → 초기화 불가능 |
| **외부 검증 필요** | ✓ (시빌 방어 불가능) | ✓ (초기 신뢰 앵커 필요) |
| **실용적 가치** | 단순한 평판 시스템 | 탐지 + 시간적 신뢰 구축 |
| **공격 저항** | 동형 그래프에 취약 | Slow attack에 취약 |

**핵심 통찰**:
- Static impossibility (정리 3.1): **"누가 시빌인지 알 수 없다"**
- Dynamic impossibility (정리 B.7): **"처음에 누구를 믿어야 할지 알 수 없다"**

두 불가능성은 **동일한 근본 문제**의 다른 표현이다: **외부 신원 검증 없이는 주소 기반 시스템에서 시빌 저항 불가능**.
...
1. **초기 신뢰 설정에 투명하라**:
   - "완전한 탈중앙화 시빌 방어"라고 주장하지 말 것
   - 초기 신뢰 앵커를 명시할 것 (예: "토큰 보유자", "POAP 소지자")

2. **동적 모델을 보조 도구로 사용하라**:
   - Primary: 진입 조건 ($\phi$) - 맥락 정의
   - Secondary: 동적 점수 ($s(v, t)$) - 평판 누적

3. **탐지 시스템으로 활용하라**:
   - 실시간 모니터링으로 급격한 공격 탐지
   - 정기적 감사로 의심스러운 패턴 제거

**SYB 시스템에서의 적용**:

현재 SYB 구현은 주로 static scoring을 사용하지만, 동적 확장이 가능하다:
...
이를 통해:
- 오래된 계정에 감쇠율 적용: $s(v, t) = s(v, t-1) \cdot e^{-\lambda \Delta t} + \Delta s$
- 급격한 점수 변화 탐지: $|s(v, t) - s(v, t-1)| > \theta_{\text{alarm}}$

**그러나 이것도 진입 조건 $\phi$가 초기 집단을 올바르게 정의했을 때만 의미가 있다 (Entry Action Framework).**
